...
std::vector<bool> CheckPercolate(const ColoredHexagon &hex) {
    ...
    std::vector<std::vector<bool>> not_visited(size * 2 - 1, std::vector<bool>(size * 2 - 1, true));
    std::vector<std::pair<int, int>> stack;
    for (auto liquid_id = 0; liquid_id < liquids; ++liquid_id) {
        for (auto &line : not_visited) {
            for (auto element : line) {
                element = true;
            }
        }
        not_visited[size - 1][size - 1] = false;
        // At this point we guarantie that stack is empty
        stack.push_back(std::make_pair(size - 1, size - 1));
        while (!stack.empty()) {
            auto cell = stack.back();
            stack.pop_back();
            for (auto &new_cell : hex.GetLiquidNeighbors(cell.first, cell.second, liquid_id)) {
                if (hex.IsInBorder(new_cell.first, new_cell.second)) {
                    results[liquid_id] = true;
                    stack.clear();
                    break;
                } else if (not_visited[new_cell.first][new_cell.second]) {
                    not_visited[new_cell.first][new_cell.second] = false;
